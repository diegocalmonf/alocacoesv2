...
function _prevPorMes(){
  const byMonth={};
  prevToArray().forEach(r=>{ const m=_mesDe(r.iso); if(!m) return; (byMonth[m]=byMonth[m]||[]).push(r); });
  return byMonth;
}
...
function _allocPorMes(){
  const byMonth={};
  allocToArray().forEach(r=>{ const m=_mesDe(r.iso); if(!m) return; (byMonth[m]=byMonth[m]||[]).push(r); });
  return byMonth;
}
...
function _lerTodosBuckets(){
  ...
  return _db.ref(ALLOC_DATA_PATH).once("value").then(s=>{
    ...
    Object.keys(v).forEach(m=>{ const a=Array.isArray(v[m])?v[m]:Object.values(v[m]||{}); a.forEach(r=>arr.push(r)); });
    if(arr.length) return arrayToAlloc(arr);
...
function _carregarJanela(meses){
  ...
  const podarFora = !_histCompleto;   // só remove meses fora da janela quando NÃO há histórico completo carregado
  ...
  alvo.forEach(m=>{
    ...
    ref.on("value", s=>{
      const v=s.val();
      const mAlloc=arrayToAlloc(Array.isArray(v)?v:Object.values(v||{}));
      Object.keys(DATA).forEach(k=>{ if(mesDe(k)===m) delete DATA[k]; });
      Object.assign(DATA, mAlloc);   // mantém o mês da janela ao vivo (sem mexer em _histCompleto)
      try{ if(typeof telaAtual!=="undefined" && telaAtual==="grade" && typeof renderAll==="function") renderAll(); }catch(e){}
    }, e=>console.warn("[janela] "+m+":", e));
  });
}
...
function _carregarJanelaPrev(meses){
  ...
  alvo.forEach(m=>{
    ...
    ref.on("value", s=>{
      const v=s.val();
      const mPrev=arrayToPrev(Array.isArray(v)?v:Object.values(v||{}));
      Object.keys(PREV).forEach(k=>{ if(mesDe(k)===m) delete PREV[k]; });
      Object.assign(PREV, mPrev);   // silencioso em Fase 1 (sem render)
    }, e=>console.warn("[previsto] "+m+":", e));
  });
}
...
function renderTorre(){
  ...
  if(torreView === "esteira"){
    ...
    if(ALLOC_WINDOWED_READ && !_histCompleto){ _garantirHistoricoCompleto().then(renderTorre); return; }
    host.innerHTML = `<div id="torreAlertHost"></div>`;
    ...
    if(ALLOC_WINDOWED_READ && !_histCompleto){ _garantirHistoricoCompleto().then(renderTorre); return; }
    host.innerHTML = `<div id="torreAderenciaHost"></div>`;
...
function openActions(){if(!canViewCadastros()){alert("Você não tem acesso a Ações & Cadastros.");return;}if(ALLOC_WINDOWED_READ && !_histCompleto){_garantirHistoricoCompleto().then(openActions);return;}try{ensureCapIntegration();}catch(e){}actTab="projetos";actEditing=null;actSearch="";el("actOverlay").classList.add("open");renderActions();}
...
function renderActions(){renderTabs();lucideRefresh();if(actTab==="usuarios"){renderUsers();return;}if(actTab==="importar"){renderImporter();return;}if(actTab==="auditoria"){renderAuditoria();return;}if(actEditing){renderForm();}else{renderList();}}
...
/* ===================== Aba Previsto no cadastro do projeto (MOTOR LIGADO) =====================
   A aba é a FONTE da previsão do projeto (p.previstoLinhas, persistido com o projeto).
   INCLUIR uma linha grava nas DUAS camadas: previsto (PREV, imutável, com `linha`=id de origem)
   e realizado (DATA, só onde o slot está livre), expandindo em dias úteis (pula fds/feriado).
   REMOVER (Opção A): o previsto PERMANECE (intocável); esvazia só o realizado dos slots ainda
   confirmados (iguais à baseline desta linha); slots editados à mão são preservados.
   Salvaguarda: _garantirHistoricoTudo() carrega DATA+PREV completos antes de escrever, para o
   persist por buckets não sobrescrever meses fora da janela. */
let _prevTabLinhas=[];
function _previstoTabHTML(p){
  const slots=SLOTS.filter(s=>!s.lunch);
  const team=(p.analistas||[]).filter(Boolean);
  const ativ=(REG.atividades||[]).filter(a=>a.ativo!==false).map(a=>a.nome);
  if(!p.nome){
  ...
  return `
  <div class="pv-proto-banner"><i data-lucide="git-compare-arrows"></i> Ao <b>incluir</b>, cada linha grava <b>previsto + realizado</b> nos dias úteis do intervalo. O previsto é imutável; <b>remover</b> uma linha esvazia só o realizado dos slots ainda confirmados (vira previsto não realizado).</div>
  <div class="pv-incl">
    <div class="pv-incl-grid">
      ...
      <div class="f"><label>Slot</label><select id="pvSlot">${slots.map(s=>`<option value="${s.id}">${s.id} · ${s.time}</option>`).join("")}</select></div>
      <div class="f"><label>Atividade</label><select id="pvAtv">${ativ.map(a=>`<option ${a==="Implantação"?"selected":""}>${enc(a)}</option>`).join("")}</select></div>
      <div class="f"><label>De</label><input type="date" id="pvIni"></div>
      <div class="f"><label>Até</label><input type="date" id="pvFim"></div>
  ...
  <div class="gp-lbl" style="margin-top:16px">Linhas de previsto declaradas</div>
  <div id="pvLinhas"></div>
  <div id="pvResumo" class="pv-resumo"></div>`;
}
...
function _prevLinhasOverlap(){
  const pares=[];
  for(let i=0;i<_prevTabLinhas.length;i++)for(let j=i+1;j<_prevTabLinhas.length;j++){
    const a=_prevTabLinhas[i],b=_prevTabLinhas[j];
    if(a.an===b.an && a.slot===b.slot && a.ini<=b.fim && b.ini<=a.fim) pares.push([i,j]);
  }
  return pares;
}
function _renderPrevLinhas(){
  ...
  if(!_prevTabLinhas.length){
    host.innerHTML=`<div class="hint" style="padding:10px">Nenhuma linha incluída ainda. Use o formulário acima para declarar o previsto do projeto.</div>`;
  ...
  let totalSlots=0;
  const rows=_prevTabLinhas.map((l,idx)=>{
    const dias=_diasUteisEntre(l.ini,l.fim).length; totalSlots+=dias;
    const di=parseISO(l.ini), df=parseISO(l.fim);
    return `<div class="pv-row${flagged.has(idx)?" pv-row-warn":""}">
      <div class="cr-an">${enc(l.an)}</div>
      <div>${enc(l.slot)}</div>
      <div>${enc(l.atv)}</div>
      <div>${fmtDM(di)}/${di.getFullYear()} – ${fmtDM(df)}/${df.getFullYear()}</div>
      <div>${dias} dia(s) útil(eis)</div>
      <button class="cr-go pv-del" data-idx="${idx}">Remover</button>
    </div>`;
  }).join("");
  host.innerHTML=`<div class="confl-list">${rows}</div>`;
  host.querySelectorAll(".pv-del").forEach(b=>b.addEventListener("click",()=>{ _pvRemoverLinha(+b.dataset.idx); }));
  if(resumo)resumo.innerHTML=`<div class="gp-stats"><span class="gp-stat new"><b>${_prevTabLinhas.length}</b> linha(s)</span><span class="gp-stat"><b>${totalSlots}</b> slot(s) de previsto (estimado, dias úteis)</span>${overlaps.length?`<span class="gp-stat conf"><b>${overlaps.length}</b> sobreposição(ões)</span>`:""}</div>`;
  if(warn){
    if(overlaps.length){ warn.style.display=""; warn.innerHTML=`<i data-lucide="alert-triangle"></i> ${overlaps.length} sobreposição(ões): o mesmo analista aparece em duas linhas no mesmo slot com datas que se cruzam.`; }
...
function _bindPrevistoTab(p, isNew){
  ...
  _prevTabLinhas = p.previstoLinhas;          // mesma referência: a aba É a fonte do previsto do projeto
  ...
  function showTab(prev){
    ...
    if(prev){
      // A pré-alocação tem permissão própria (prealoc), independente do nível de Cadastros:
      // reabilita (ou trava) os campos da aba conforme canEditAction("prealoc").
      const podeEditar=canEditAction("prealoc");
      ["pvAn","pvSlot","pvAtv","pvIni","pvFim","pvAdd"].forEach(id=>{ const e=el(id); if(e) e.disabled=!podeEditar; });
    }
  }
  if(tabDados)tabDados.addEventListener("click",()=>showTab(false));
  if(tabPrev)tabPrev.addEventListener("click",()=>{ showTab(true); lucideRefresh(); });
  const add=el("pvAdd");
  if(add)add.addEventListener("click",()=>{
    ...
    const slot=el("pvSlot").value, atv=el("pvAtv").value, ini=el("pvIni").value, fim=el("pvFim").value;
    ...
    if(fim<ini){alert("A data final não pode ser anterior à inicial.");return;}
    _pvIncluirLinha(p, {an,slot,atv,ini,fim});
  });
  _renderPrevLinhas();
}
// Garante DATA e PREV completos em memória antes de escrever (evita clobber de buckets fora da janela).
...
function _pvExpandir(l){ return _diasUteisEntre(l.ini,l.fim).map(iso=>({iso, k:key(l.an,iso,l.slot)})); }
function _pvNovoId(){ return "L"+Date.now().toString(36)+Math.random().toString(36).slice(2,6); }
// INCLUIR: grava previsto (uma vez, imutável) + realizado (só se o slot estiver livre).
function _pvIncluirLinha(p, dados){
  _garantirHistoricoTudo().then(()=>{
    const l=Object.assign({id:_pvNovoId(), criadoEm:new Date().toISOString(), criadoPor:(_currentUser&&_currentUser.email)||"sistema"}, dados);
    const cells=_pvExpandir(l);
    if(!cells.length){ alert("O intervalo não tem nenhum dia útil (só fim de semana/feriado)."); return; }
    let preencheReal=0, ocupadoOutro=0, jaPrev=0;
    cells.forEach(({k})=>{
      if(_temConteudo(PREV[k])) jaPrev++;
      const real=DATA[k];
      if(!_temConteudo(real)) preencheReal++;
      else if(_normProj(real.cliente)!==_normProj(p.nome)) ocupadoOutro++;
    });
    const sobre=(p.previstoLinhas||[]).some(x=>x.an===l.an&&x.slot===l.slot&&x.ini<=l.fim&&l.ini<=x.fim);
    let msg=`Incluir previsto de "${p.nome}":\n• ${cells.length} dia(s) útil(eis) · ${l.an} · ${l.slot}\n• ${preencheReal} slot(s) de realizado serão preenchidos (livres)`;
    if(ocupadoOutro) msg+=`\n• ${ocupadoOutro} slot(s) já ocupados por outro projeto serão PRESERVADOS (vira divergência)`;
    if(jaPrev) msg+=`\n• ${jaPrev} slot(s) já tinham previsto e serão mantidos (previsto é imutável)`;
    if(sobre) msg+=`\n\n⚠ Sobreposição: já existe linha do mesmo analista/slot com datas que se cruzam.`;
    if(!confirm(msg)) return;
    cells.forEach(({k})=>{
      if(!_temConteudo(PREV[k])) PREV[k]={cliente:p.nome, atividade:l.atv, origem:"projeto:"+p.nome, linha:l.id};
      if(!_temConteudo(DATA[k])) DATA[k]={cliente:p.nome, atividade:l.atv};
    });
    p.previstoLinhas.push(l);
    persist(); persistPrev();
    try{ audit("project.previsto.add", p.nome, null, l); }catch(e){}
    const i1=el("pvIni"),i2=el("pvFim"); if(i1)i1.value=""; if(i2)i2.value="";
    _renderPrevLinhas(); renderAll();
  });
}
// REMOVER (Opção A): previsto permanece (imutável); esvazia só o realizado dos slots ainda confirmados.
function _pvRemoverLinha(idx){
  ...
  _garantirHistoricoTudo().then(()=>{
    const cells=_pvExpandir(l);
    const aEsvaziar=[]; let preservados=0;
    cells.forEach(({k})=>{
      const pv=PREV[k]; if(!pv || pv.linha!==l.id) return;     // só os slots que ESTA linha criou
      const real=DATA[k];
      const confirmado = real && _normProj(real.cliente)===_normProj(p.nome) && (real.atividade||"")===(l.atv||"");
      if(confirmado) aEsvaziar.push(k);                         // intocado = igual à baseline
      else if(_temConteudo(real)) preservados++;               // editado à mão → não toca
    });
    if(!confirm(`Remover esta linha de previsto?\n• ${aEsvaziar.length} slot(s) de realizado serão esvaziados (viram previsto não realizado)\n• ${preservados} slot(s) editados à mão serão preservados\n• O previsto (baseline) permanece — é imutável.`)) return;
    aEsvaziar.forEach(k=>delete DATA[k]);                       // Opção A
    p.previstoLinhas.splice(idx,1);
    persist();                                                 // PREV não muda → não precisa persistPrev
    try{ audit("project.previsto.remove", p.nome, l, null); }catch(e){}
    _renderPrevLinhas(); renderAll();
  });
}
...
function openReports(){
  ...
  if(ALLOC_WINDOWED_READ && !_histCompleto){ _garantirHistoricoCompleto().then(openReports); return; }
  repTab="alocacao";
...
function openConflitos(){
  ...
  const body=el("conflitosBody"); if(body) body.innerHTML=`<div class="loading">Carregando previsto e realizado…</div>`;
  el("conflitosOverlay").classList.add("open");
  // precisa do histórico completo das DUAS camadas para listar tudo
  Promise.all([_garantirHistoricoCompleto(), _garantirHistoricoCompletoPrev()])
    .then(()=>{ renderConflitos(); lucideRefresh(); })
    .catch(()=>{ renderConflitos(); lucideRefresh(); });
}
...
function renderRepAderencia(){
  if(ALLOC_WINDOWED_READ && !_histCompletoPrev){
    el("repBody").innerHTML='<div class="loading">Carregando previsto…</div>';
    _garantirHistoricoCompletoPrev().then(renderReports); return;
  }
  const dias=repDays(); const ns=repAnalysts();
  if(!dias.length){ el("repBody").innerHTML='<div class="rep-empty">Selecione um período válido.</div>'; _lastRepRows=[]; return; }
  const work=SLOTS.filter(s=>!s.lunch);
...
function openKPIs(){
  ...
  if(ALLOC_WINDOWED_READ && !_histCompleto){ _garantirHistoricoCompleto().then(openKPIs); return; }
  if(canViewAction("prealoc") && ALLOC_WINDOWED_READ && !_histCompletoPrev){ _garantirHistoricoCompletoPrev().then(openKPIs); return; }
